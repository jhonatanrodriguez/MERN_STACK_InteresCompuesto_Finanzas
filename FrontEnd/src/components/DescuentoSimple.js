...
const renderCustomizedLabel = ({cx,cy,midAngle,innerRadius,outerRadius,percent,index}) => {
 ...
 function DescuentoSimple() {
    const [data, setData] = useState([]);
    useEffect(() => {
        const dataFetch = async () => {
            try {
                const response = await fetch("http://localhost:3000/simdes");
            ...
            <NavBar/>
             <div class="flex flex-row bg-gray-100 ">
                 <div class="m-4">
                     ...
                     <h1 class="text-[#949190] text-3xl">SIMPLE</h1>
                 </div>
             ...
             <div class="flex flex-row justify-center bg-blue-50">
                 <div class="flex flex-col justify-center items-center w-3/4 pl-10">
                     <h1 class="mb-5 font-semibold text-[#3A5862] ">CLIENTE VS CAPITAL</h1>
                     <ResponsiveContainer width={"100%"} height={200}>
                         <BarChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                             ...
                             <Tooltip />
                             <Legend />
                             <Bar dataKey="interes" fill="#F46D29" name="Monto" activeBar={<Rectangle fill="pink" stroke="blue" />} />
                             <Bar dataKey="capital" fill="#3A5862" name="Interes" activeBar={<Rectangle fill="gold" stroke="purple" />} />
                             <Bar dataKey="monto" fill="#34BDCF" name="Capital" activeBar={<Rectangle fill="pink" stroke="blue" />} />
                         </BarChart>
                     </ResponsiveContainer>
                 </div>
                 <div class="flex flex-col justify-center items-center w-1/2">
                     <PieChart width={400} height={400}>
                         <Pie data={data} cx={200} cy={200} labelLine={false} label={renderCustomizedLabel} name="capital"
                             outerRadius={120} fill="#8884d8" dataKey="capital" isAnimationActive={false}>
                        ...
                        {data?.map((item, i) => {
                            return (
                                <tr key={i + 1} class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                    ...
                                    <td class="px-6 py-4 text-center">$ {item.monto} </td>
                                    ...
                                    <td class="px-6 py-4 text-center">$ {item.capital.toFixed(2)} </td>
                                </tr>
                            );
                        })};
                </tbody>
